...
class CourseListView(View):
    ...
    def get(self, request):
        ...
        # 热门课程推荐
        hot_courses = Course.objects.all().order_by('-click_count')[:3]
        # 搜索功能
        ...
        if sort:
            if sort == "students":
                ...
                all_courses = all_courses.order_by("-click_count")
        # 分页
        try:
...
class CourseDetailView(View):
    ...
    def get(self, request, course_id):
        ...
        if request.user.is_authenticated:
            if request.user.UserFavorite.objects.filter(user=request.user, fav_id=course.id, fav_type=1):
                has_fav_course = True
            if request.user.UserFavorite.objects.filter(user=request.user, fav_id=course.course_org.id, fav_type=2):
                has_fav_org = True
        tag = course.tag
        if tag:
            # 需要从1开始不然会推荐自己
...
class AddCommentsView(View):
    ...
    def post(self, request):
        ...
        if int(course_id) > 0 and comments:
            ...
            course_comments = UserComment()
            # 获取评论的是哪门课程
            course = Course.objects.get(id = int(course_id))
            # 分别把评论的课程、评论的内容和评论的用户保存到数据库
            course_comments.course = course
            course_comments.comments = comments
            course_comments.user_profile = request.user
