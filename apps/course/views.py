...
class CourseDetailView(View):
    ...
    def get(self, request, course_id):
        ...
        if request.user.is_authenticated:
            if request.user.favourite_course.filter(favouritecourse__course=course):
                has_fav_course = True
            if request.user.favourite_organization.filter(favouriteorganization__organization=course.course_org):
                has_fav_org = True
        tag = course.tag
        if tag:
            # 需要从1开始不然会推荐自己
